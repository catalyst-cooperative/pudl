"""
To run this script: `python notebooks/work-in-progress/xbrl_dagster_prototypes/config_option.py`

or to view in Dagit:
`dagit -f notebooks/work-in-progress/xbrl_dagster_prototypes/config_option.py`
"""
import pandas as pd
from dagster import Field, GraphOut, graph, in_process_executor, job, op
from sqlalchemy import column

#### Cleaning metadata
"""
This could be created from pydantic metadata classes.
An obvious con here dagster doesn't allow for duplicate op names
so when a generic cleaning op is reused, it gets an auto incremented suffix.
See rename_columns and rename_columns_2.
"""
TRANSFORM_METADATA = {
    "ops": {
        "transform": {
            "ops": {
                "transform_steam": {
                    "ops": {
                        "rename_columns": {
                            "config": {"column_mapping": {"col": "column"}}
                        },
                        "rename_columns_2": {
                            "config": {"column_mapping": {"col": "column"}}
                        },
...
@op(
    config_schema={
        "column_mapping": Field(
            dict, description="Mapping old to new column names.", is_required=True
        )
    }
)
def rename_columns(context, df: pd.DataFrame) -> pd.DataFrame:
    """Rename dataframe columns."""
    # Access the config params via the dagster context keyword.
    return df.rename(columns=context.op_config["column_mapping"])
...
def transform_steam(dbf_raw_dfs, xbrl_raw_dfs, transformed_fuel_table) -> pd.DataFrame:
    ...
    transformed_dbf_df = rename_columns(steam_dbf_df)
    transformed_xbrl_df = rename_columns(steam_xbrl_df)
...
def transform_fuel(dbf_raw_dfs, xbrl_raw_dfs):
    ...
    transformed_dbf_df = rename_columns(fuel_dbf_df)
    transformed_xbrl_df = rename_columns(fuel_xbrl_df)
...
def transform_hydro(dbf_raw_dfs, xbrl_raw_dfs):
    ...
    transformed_dbf_df = rename_columns(hydro_dbf_df)
    transformed_xbrl_df = rename_columns(hydro_xbrl_df)
...
def main():
    etl_ferc1_job.execute_in_process(run_config=TRANSFORM_METADATA)
