...
function notify_slack() {
    ...
    message+="$(slack_stage_status "Pass Data Validations (FKs/dbt)" "$DATA_VALIDATION_STATUS" "$DATA_VALIDATION_DURATION")\n"
    ...
    message+="$(slack_stage_status "Redeploy PUDL Data Viewer" "$DEPLOY_EEL_HOLE_STATUS" "$DEPLOY_EEL_HOLE_DURATION")\n"
    message+="$(slack_stage_status "Make GCS Outputs Read-Only" "$GCS_TEMPORARY_HOLD_STATUS" "$GCS_TEMPORARY_HOLD_DURATION")\n\n"
