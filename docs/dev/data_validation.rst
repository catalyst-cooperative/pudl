...
Sometimes you will want to test a property that can't be expressed using the
existing DBT tests like ``check_row_counts_per_partition`` or the tests in
``dbt_expectations`` or ``dbt_utils``.

In those cases, you can use `Dagster's asset checks
<https://docs.dagster.io/guides/test/asset-checks>`__ for a pure Python
approach, or you can define a new test within DBT.
...
be quite performant. But, they're a little less flexible than the asset checks,
so sometimes you will just have to write the check in Python.

How do I write a new DBT test?
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

A DBT test is a templated SQL query that runs on your output data to look for
problems in the data. If the query returns no rows, then the test passes. If
the query returns any rows at all, then there are problems with the data and the
test fails.

The test lives as a templated piece of SQL within
...
4. Write a SQL ``SELECT`` statement that returns any data that would fail your
   test, as well as useful debugging information
...
1. Copy the test file from above to ``pudl/dbt/macros/your_macro.sql``
...
wrapper around the logic macro you just wrote. Change the test file to read
like this::
