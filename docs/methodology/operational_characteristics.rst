...
empirically from :doc:`/data_sources/epacems` hourly generation and fuel data, in the
:ref:`out_epacems__yearly_operational_characteristics` table.

This methodology and its original implementation were generously contributed to PUDL by
`Sylvan Energy <https://sylvan.energy>`__, with support for integration from `GridLab
<https://gridlab.org>`__,. We've adapted Sylvan Energy's analysis to run automatically
...
suit your use case. :ref:`feedback-welcome`!
...
:ref:`core_epa__assn_eia_epacamd` if you need to connect these characteristics to EIA
