...
def connect_db(pudl_settings):
    ...
    Args:
        pudl_settings (dict or None): A dictionary which defines resources used
            by PUDL given user defined values for ``PUDL_IN`` and ``PUDL_OUT``,
            including the SQLite URL for the FERC Form 1 database. If None, a
            ``pudl_settings`` dictionary is constructed based on the values for
            ``PUDL_IN`` and ``PUDL_OUT`` that are read from the user's
            ``.pudl.yml`` file.
        testing (bool): If True, access the PUDL testing database instead of
            the (default) live database meant for daily use.
    ...
    return sa.create_engine(pudl_settings['pudl_sqlite_url'])
