...
def get_fk_list(engine: sa.Engine, table: str) -> pd.DataFrame:
...
def check_foreign_keys(sqlite_uri: str, timeout: float = 1_000.0) -> None:
    ...
    Args:
        sqlite_uri: sqlite:///... style path to the database.
        timeout: connection timeout for the database connection.

    Raises:
        ForeignKeyErrors: if data in the database violate foreign key constraints.
    """
    logger.info(f"Checking foreign key constraints on {sqlite_uri}.")
    engine = sa.create_engine(sqlite_uri, connect_args={"timeout": timeout})
    with engine.begin() as con:
        ...
        if not fk_errors.empty:
            ...
            table_foreign_keys = pd.concat(
                [get_fk_list(engine, table) for table in tables_with_fk_errors]
            )
