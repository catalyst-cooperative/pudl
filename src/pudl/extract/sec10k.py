...
the data for testing, so the :class:`Sec10kDataConfig` allow specification of which years
...
from pudl.settings import DataSource, Sec10kDataConfig
...
def extract(ds: Datastore, table: str, years: list[int]) -> pd.DataFrame:
    ...
    # Filter the dataframe for years specified by the data config:
...
def raw_sec10k_asset_factory(table) -> dg.AssetsDefinition:
    ...
    def sec10k_asset(context) -> pd.DataFrame:
        sec10k_data_config: Sec10kDataConfig = (
            context.resources.global_data_config.pudl.sec10k
        )
        ds: Datastore = context.resources.datastore
        if table in sec10k_data_config.tables:
            return extract(ds=ds, table=table, years=sec10k_data_config.years)
