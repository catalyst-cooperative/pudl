...
def connect_db(pudl_settings=None, testing=False):
    """
    Create an SQL Alchemy engine for connecting to the FERC Form 1 SQLite DB.
    ...
    where ``PUDL_OUT`` is a directory chosen by the user to contain the output
    generated by PUDL. This is one of several values stored in the
    ...
    Args:
        pudl_settings (dict or None): A dictionary which defines resources used
            ...
            ``.pudl.yml`` file. testing (bool): If True, access the FERC Form 1
            testing database instead of the (default) live database meant for
            daily use.

    Returns:
        sa.engine.Engine: An SQL Alchemy database engine allowing access to the
            relevant (live or testing) FERC Form 1 SQLite database.
...
def _create_tables(engine, md):
    """
    Create the FERC Form 1 DB tables in the SQLite database.

    Uses the database schema described by the MetaData object ``md`` to
    instantiate the FERC Form 1 database, via ``engine``.

    Args:
        engine (sa.engine.Engine): An SQL Alchemy SQLite database Engine
            pointing at the location where the SQLite database should be
            created.
        md (sa.schema.MetaData): An SQL Alchemy MetaData object describing the
            structure and datatypes of the FERC Form 1 database to be created.
...
def add_sqlite_table(table_name, sqlite_meta, dbc_map, data_dir,
    ...
    Create a new sa.Table object named ``table_name`` and add it to the
