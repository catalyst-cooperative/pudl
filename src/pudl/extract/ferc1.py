...
def extract_dbf(
    ...
    for pudl_table in ferc1_settings.tables:
        if pudl_table not in pudl.TABLE_NAME_MAP_FERC1:
            raise ValueError(
        ...
        if pudl_table == "income_statement_ferc1":
            ...
            income_tbls = []
            for raw_income_table_name in pudl.TABLE_NAME_MAP_FERC1[pudl_table]["dbf"]:
                income_tbls.append(
                    extract_dbf_generic(
            ...
            ferc1_raw_dfs[pudl_table] = extract_dbf_generic(
                ferc1_engine=sa.create_engine(pudl_settings["ferc1_db"]),
                ferc1_settings=ferc1_settings,
                table_name=pudl.TABLE_NAME_MAP_FERC1[pudl_table]["dbf"],
            )
...
def extract_xbrl(
    ...
    for pudl_table in ferc1_settings.tables:
        ...
        if "xbrl" not in pudl.TABLE_NAME_MAP_FERC1[pudl_table]:
        ...
        xbrl_table = pudl.TABLE_NAME_MAP_FERC1[pudl_table]["xbrl"]
