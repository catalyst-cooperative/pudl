"""Canonical Dagster registry assembly for PUDL."""
...
# Limit the number of concurrent workers when launching assets that use a lot of memory.
...
def build_defs(
    *,
    ...
    This allows tests and other callers to reuse the canonical Dagster registry from
    ``pudl.defs`` while supplying concrete resources or a narrowed set of definitions
    without mutating the module-level production ``defs`` object.
    """
    ...
    if resource_overrides:
        ...
        if etl_settings_override is not None:
            # These special cases for the FERC IO managers are a temporary workaround.
            ...
            # PR we will get rid of the IO manager wrappers and their nested resource
            # dependencies, which will make this kind of override much more
            # straightforward. For now, this lets us assemble these interdependent
...
"""The canonical Dagster definitions assembled by ``pudl.defs``."""
