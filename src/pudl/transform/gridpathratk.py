...
@asset(io_manager_key="pudl_io_manager")
def core_gridpathratk__profile_aggregations(
    raw_gridpathratk__wind_plant_aggregations: pd.DataFrame,
    raw_gridpathratk__solar_generator_aggregations: pd.DataFrame,
) -> pd.DataFrame:
    """Transform and combine raw GridPath RA Toolkit generator aggregations."""
    return pd.concat(
        [
            _transform_aggs(raw_gridpathratk__wind_plant_aggregations),
            _transform_aggs(raw_gridpathratk__solar_generator_aggregations),
        ]
    )
