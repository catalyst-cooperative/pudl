...
from pudl.helpers import zero_pad_numeric_string, standardize_phone_column, fix_na, standardize_state_columns
...
@asset(
    ins={
        "raw_data": AssetIn(key="raw_phmsagas__yearly_distribution")
    },
...
def core_phmsagas__yearly_distribution_operators(
    ...
    df[YEARLY_DISTRIBUTION_OPERATORS_COLUMNS["columns_to_convert_to_ints"]] = df[YEARLY_DISTRIBUTION_OPERATORS_COLUMNS["columns_to_convert_to_ints"]].astype("Int64")
    ...
    df[YEARLY_DISTRIBUTION_OPERATORS_COLUMNS["columns_with_nas_to_fill"]] = df[YEARLY_DISTRIBUTION_OPERATORS_COLUMNS["columns_with_nas_to_fill"]].fillna(0)
    ...
    df = standardize_state_columns(df, YEARLY_DISTRIBUTION_OPERATORS_COLUMNS["cols_for_state_standardization"])
