...
class UTCTimeseriesDataFrame(pa.DataFrameModel):
    ...
    converted to a ``AligneTimeseriesDataFrame``, then pivoted
...
class AligneTimeseriesDataFrame(pa.DataFrameModel):
    """Define schema of input tables for timeseries cleaning.

    This model is nearly identical to a ``UTCTimeseriesDataFrame``, but
...
class TimeseriesMatrix(pa.DataFrameModel):
    ...
    generated by pivoting a ``AligneTimeseriesDataFrame``, this
...
) -> DataFrame[AligneTimeseriesDataFrame]:
...
@pa.check_types
def pivot_localized_timeseries_dataframe(
    localized_df: DataFrame[AligneTimeseriesDataFrame],
) -> DataFrame[TimeseriesMatrix]:
    """Pivot localized timeseries dataframe into timeseries matrix."""
    return localized_df.pivot(index="datetime", columns="id_col", values="value_col")
...
def impute_timeseries_asset_factory(
    ...
    def _prepare_timeseries_matrix(
        ...
        localized_df = utc_dataframe_to_aligned(
            input_df.rename(columns={value_col: "value_col", id_col: "id_col"})
        )

        # Pivot to timeseries matrix
        matrix = pivot_localized_timeseries_dataframe(localized_df)
