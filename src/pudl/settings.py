...
class DatasetsSettings(BaseModel):
    ...
    def get_datasets(cls):  # noqa: N805
        ...
        return {dataset: settings for dataset, settings in vars(cls).items() if settings is not None}
...
class EtlSettings(BaseSettings):
    ...
    def from_yaml(cls, path: str):
        ...
        """
        with fsspec.open(path, "r") as fs:
            yaml_file = yaml.safe_load(fs)
        return cls.parse_obj(yaml_file)

    def write_yaml(self, path: str):
        """Write an ETLSettings object to a yaml file."""
        with fsspec.open(path, "w") as outfile:
