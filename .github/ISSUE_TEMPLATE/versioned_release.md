...
- [ ] Verify that the corresponding `zenodo-data-release` GHA completed successfully. If it times out, re-run manually (production, vYYYY.M.x, default regex, no-publish) until it does. If Zenodo is extra cranky, ignore the GHA and upload the files manually.
