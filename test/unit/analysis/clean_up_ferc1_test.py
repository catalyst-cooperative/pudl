...
from pudl.analysis.clean_up_ferc1 import (
    associate_notes_with_values_sg,
...
def test_remove_bad_rows_sg(df_update):
    ...
    assert (
        remove_bad_rows_sg(remove_bad_rows_input).empty is True
    ), "Test DataFrame should be empty"
...
def test_label_row_type_sg():
    ...
    label_rows_actual = label_row_type_sg(label_rows_input).pipe(
...
def test_improve_plant_type_sg_1():
    ...
    improve_plant_type_actual = improve_plant_type_sg(improve_plant_type_input).pipe(
        apply_pudl_dtypes, group="ferc1"
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            pd.DataFrame({"plant_name_ferc1": ["ferc"]}), marks=pytest.mark.xfail
        ),
        pytest.param(
            pd.DataFrame({"plant_name_ferc1": ["ferc page 8888"]}),
            marks=pytest.mark.xfail,
        ),
        pytest.param(
            pd.DataFrame({"plant_name_ferc1": ["ferc 2001"]}), marks=pytest.mark.xfail
        ),
        pytest.param(
            pd.DataFrame(
                {"plant_name_ferc1": ["ferc page 8888"], "plant_type": ["wind"]}
            ),
            marks=pytest.mark.xfail,
        ),
    ],
...
def test_remove_header_note_rows_sg():
    ...
    remove_header_note_rows_actual = remove_header_note_rows_sg(
        remove_header_note_rows_input
    )
