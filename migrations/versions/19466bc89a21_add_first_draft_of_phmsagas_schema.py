...
Revision ID: 19466bc89a21
Revises: 76397c4db652
Create Date: 2025-09-16 16:24:28.007604
...
def upgrade() -> None:
    ...
    sa.PrimaryKeyConstraint('report_number', 'install_decade', name=op.f('pk__core_phmsagas__yearly_distribution_by_install_decade'))
    )
    ...
    sa.Column('material', sa.Enum('unprotected_steel_bare', 'unprotected_steel_coated', 'cathodically_protected_steel_bare', 'cathodically_protected_steel_coated', 'steel', 'pvc', 'pe', 'abs', 'other_plastic', 'plastic', 'cast_or_wrought_iron', 'wrought_iron', 'ductile_iron', 'copper', 'other_alt', 'other_material', 'other', 'reconditioned_cast_iron', 'cast_iron', 'all_materials', 'total'), nullable=False, comment='The material of the gas distribution pipe. The categories of material types have changed slightly over the years (ex: cast and wrought iron were broken up in two categories before 1984).'),
    sa.Column('mains_miles', sa.Float(), nullable=True, comment='The miles of mains distribution pipeline.'),
    sa.Column('services', sa.Float(), nullable=True, comment='Number of end in system at end of year.'),
    sa.PrimaryKeyConstraint('report_number', 'material', name=op.f('pk__core_phmsagas__yearly_distribution_by_material'))
    )
    ...
    sa.PrimaryKeyConstraint('report_number', 'main_size', 'material', name=op.f('pk__core_phmsagas__yearly_distribution_by_material_and_size'))
    )
    ...
    sa.PrimaryKeyConstraint('report_number', 'damage_type', 'damage_sub_type', name=op.f('pk__core_phmsagas__yearly_distribution_excavation_damages'))
    )
    ...
    sa.Column('commodity', sa.Text(), nullable=True, comment='The type of gas delivered by the distribution pipeline.'),
    sa.Column('operating_state', sa.Text(), nullable=True, comment='State that the distribution utility is reporting for. Prior to 2004, this may be a list of states.'),
    sa.Column('leak_severity', sa.Enum('all_leaks', 'hazardous_leaks'), nullable=False, comment='Whether or not the leak described in this record are all leaks or hazardous leaks.'),
    sa.Column('leak_source', sa.Enum('construction_defect', 'corrosion_failure', 'equipment_failure', 'excavation_damage', 'incorrect_operation', 'material_defect', 'natural_force_damage', 'other', 'other_outside_force', 'outside_force', 'pipe_weld_joint_failure', 'third_party', 'total'), nullable=False, comment='The cause of the leaks.'),
    sa.Column('mains_miles', sa.Float(), nullable=True, comment='The miles of mains distribution pipeline.'),
    sa.Column('services', sa.Float(), nullable=True, comment='Number of end in system at end of year.'),
    sa.PrimaryKeyConstraint('report_number', 'leak_severity', 'leak_source', name=op.f('pk__core_phmsagas__yearly_distribution_leaks'))
    )
    ...
    sa.PrimaryKeyConstraint('report_number', name=op.f('pk__core_phmsagas__yearly_distribution_misc'))
    )
