"""updates to sec ownership tables

Revision ID: 60f1cd56406b
Revises: b2f5c3b44300
Create Date: 2025-05-15 10:21:18.239219
...
def upgrade() -> None:
    ...
    with op.batch_alter_table('core_sec10k__quarterly_exhibit_21_company_ownership', schema=None) as batch_op:
        batch_op.add_column(sa.Column('subsidiary_company_id_sec10k', sa.Text(), nullable=False, comment="PUDL-assigned ID for subsidiaries found in SEC 10-K Exhibit 21.\n\nThe ID is created by concatenating the CIK of the company whose filing the subsidiary\nwas found in, the subsidiary company's name, and location of incorporation. It is not\nguaranteed to be stable across different releases of PUDL and so should never be\nhard-coded in analyses."))
