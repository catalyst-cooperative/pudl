...
Revision ID: 688cca8a6bd1
Revises: b2f5c3b44300
Create Date: 2025-05-28 16:55:36.449694
...
def upgrade() -> None:
    ...
    sa.Column('energy_use_sector', sa.Enum('transportation', 'electric_power', 'other', 'total', 'commercial_other', 'industrial', 'residential', 'unspecified', 'commercial'), nullable=False, comment='Sector for energy use figures in AEO Table 2. Similar to customer class, but with some missing and some extra values.'),
    sa.Column('energy_use_type', sa.Text(), nullable=False, comment='Type of energy use, indicating the name of the series from AEO Table 2. Includes fuels, electricity, losses, and various subtotals; consult table documentation for aggregation guidelines.'),
    ...
    op.drop_table('_alembic_tmp_core_eiaaeo__yearly_projected_deliver')
    # ### end Alembic commands ###


def downgrade() -> None:
    # ### commands auto generated by Alembic - please adjust! ###
    op.create_table('_alembic_tmp_core_eiaaeo__yearly_projected_deliver',
    sa.Column('report_year', sa.INTEGER(), nullable=False),
    sa.Column('energy_use_type', sa.TEXT(), nullable=False),
    sa.Column('energy_use_mmbtu', sa.FLOAT(), nullable=True),
    sa.Column('region_name_eiaaeo', sa.TEXT(), nullable=False),
    sa.Column('region_type_eiaaeo', sa.VARCHAR(length=18), nullable=True),
    sa.Column('model_case_eiaaeo', sa.VARCHAR(length=47), nullable=False),
    sa.Column('projection_year', sa.INTEGER(), nullable=False),
    sa.Column('customer_class', sa.VARCHAR(length=17), nullable=False),
    sa.PrimaryKeyConstraint('report_year', 'region_name_eiaaeo', 'model_case_eiaaeo', 'projection_year', 'customer_class', name='pk_core_eiaaeo__yearly_projected_delivered_energy')
    )
