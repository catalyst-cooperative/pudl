"""update core and raw sec company info tables

Revision ID: 88ffc61347c2
Revises:
Create Date: 2025-02-25 14:08:15.182268
...
def upgrade() -> None:
    ...
    sa.Column('label', sa.Text(), nullable=True, comment='Longer human-readable code using snake_case'),
    sa.Column('description', sa.Text(), nullable=True, comment='Long human-readable description of the meaning of a code/label.'),
    sa.Column('balancing_authority_region_code_eia', sa.Enum('SE', 'TEX', 'CAL', 'FLA', 'SW', 'NY', 'MIDW', 'CAN', 'NW', 'CAR', 'NE', 'CENT', 'MEX', 'MIDA'), nullable=True, comment='EIA balancing authority region code.'),
    ...
    sa.Column('technology_description', sa.Enum('Utility-Scale Battery Storage', 'Coal_Retrofits', 'Hydropower', 'CSP', 'Residential Battery Storage', 'Utility-Scale PV-Plus-Battery', 'DistributedWind', 'NaturalGas_FE', 'Coal_FE', 'Geothermal', 'Biopower', 'OffShoreWind', 'UtilityPV', 'Commercial Battery Storage', 'Nuclear', 'AEO', 'LandbasedWind', 'CommPV', 'NaturalGas_Retrofits', 'ResPV', 'Pumped Storage Hydropower'), nullable=True, comment='High level description of the technology used by the generator to produce electricity.'),
    ...
    sa.Column('projection_year', sa.Integer(), nullable=False, comment='The year of the projected value.'),
    sa.Column('technology_description', sa.Enum('Utility-Scale Battery Storage', 'Coal_Retrofits', 'Hydropower', 'CSP', 'Residential Battery Storage', 'Utility-Scale PV-Plus-Battery', 'DistributedWind', 'NaturalGas_FE', 'Coal_FE', 'Geothermal', 'Biopower', 'OffShoreWind', 'UtilityPV', 'Commercial Battery Storage', 'Nuclear', 'AEO', 'LandbasedWind', 'CommPV', 'NaturalGas_Retrofits', 'ResPV', 'Pumped Storage Hydropower'), nullable=False, comment='High level description of the technology used by the generator to produce electricity.'),
    ...
    sa.Column('datasource', sa.Enum('censusdp1tract', 'censuspep', 'eia176', 'eia191', 'eia757a', 'eia860', 'eia860m', 'eia861', 'eia923', 'eia930', 'eiaaeo', 'eia_bulk_elec', 'eiawater', 'epacems', 'epacamd_eia', 'ferc1', 'ferc2', 'ferc6', 'ferc60', 'ferc714', 'ferceqr', 'gridpathratoolkit', 'mshamines', 'nrelatb', 'phmsagas', 'pudl', 'sec10k', 'vcerare'), nullable=False, comment='Code identifying a dataset available within PUDL.'),
    ...
    sa.Column('subdivision_code', sa.Enum('KS', 'MT', 'YT', 'QC', 'GA', 'IA', 'AZ', 'IL', 'OK', 'MD', 'SD', 'CA', 'AL', 'GU', 'AS', 'NB', 'NH', 'NT', 'CO', 'FL', 'AK', 'KY', 'IN', 'DE', 'OR', 'AB', 'ME', 'RI', 'MN', 'NV', 'BC', 'MB', 'NS', 'MO', 'PA', 'AR', 'ID', 'WV', 'NL', 'DC', 'HI', 'MI', 'SK', 'ON', 'NM', 'NC', 'ND', 'PR', 'TN', 'UT', 'VA', 'WI', 'VI', 'NJ', 'NY', 'LA', 'CT', 'PE', 'NU', 'OH', 'TX', 'MP', 'VT', 'WA', 'NE', 'WY', 'MS', 'MA', 'SC'), nullable=False, comment='Two-letter ISO-3166 political subdivision code (e.g. US state or Canadian provice abbreviations like CA or AB).'),
    ...
    sa.Column('zip_code', sa.Text(), nullable=True, comment='Five digit US Zip Code.'),
    sa.PrimaryKeyConstraint('central_index_key', 'report_date', name=op.f('pk_core_sec10k__quarterly_company_information'))
    )
    ...
    sa.Column('respondent_id_ferc714', sa.Integer(), nullable=True, comment='PUDL-assigned identifying a respondent to FERC Form 714. This ID associates natively reported respondent IDs from the orignal CSV and XBRL data sources.'),
    sa.Column('respondent_name_ferc714', sa.Text(), nullable=True, comment='Name of the utility, balancing area authority, or planning authority responding to FERC Form 714.'),
    sa.Column('report_date', sa.Date(), nullable=True, comment='Date reported.'),
    ...
    sa.ForeignKeyConstraint(['utility_id_eia'], ['core_eia__entity_utilities.utility_id_eia'], name=op.f('fk_out_sec10k__parents_and_subsidiaries_utility_id_eia_core_eia__entity_utilities'))
    )
    ...
    sa.Column('report_year', sa.Integer(), nullable=False, comment='Four-digit year in which the data was reported.'),
    sa.Column('utility_id_ferc1', sa.Integer(), nullable=False, comment='PUDL-assigned utility ID, identifying a FERC1 utility. This is an auto-incremented ID and is not expected to be stable from year to year.'),
    sa.Column('amount_type', sa.Enum('net_increase_decrease_in_inventory_investing_activities', 'disposition_of_investments_in_and_advances_to_associated_and_subsidiary_companies_abstract', 'net_increase_in_short_term_debt', 'net_increase_decrease_in_payables_and_accrued_expenses_investing_activities', 'net_decrease_in_short_term_debt', 'net_increase_decrease_in_cash_and_cash_equivalents_abstract', 'net_income_loss_correction', 'allowance_for_other_funds_used_during_construction_operating_activities', 'cash_provided_by_outside_sources', 'net_increase_decrease_in_allowances_held_for_speculation_investing_activities', 'collections_on_loans', 'net_increase_decrease_in_cash_and_cash_equivalents', 'payments_for_retirement_of_long_term_debt_financing_activities', 'contributions_and_advances_from_associated_and_subsidiary_companies', 'cash_flows_provided_from_used_in_investment_activities', 'net_increase_decrease_in_receivables_investing_activities', 'net_increase_decrease_in_other_regulatory_assets_operating_activities', 'starting_balance', 'net_increase_decrease_in_payables_and_accrued_expenses_operating_activities', 'other_adjustments_to_cash_flows_from_financing_activities', 'cash_flows_provided_from_used_in_investment_activities_correction', 'dividends_on_common_stock', 'allowance_for_other_funds_used_during_construction_investing_activities', 'deferred_income_taxes_net', 'dividends_on_preferred_stock', 'other_adjustments_by_outside_sources_to_cash_flows_from_financing_activities', 'gross_additions_to_nuclear_fuel_investing_activities', 'gross_additions_to_common_utility_plant_investing_activities', 'noncash_adjustments_to_cash_flows_from_operating_activities', 'other_retirements_of_balances_impacting_cash_flows_from_financing_activities', 'proceeds_from_disposal_of_noncurrent_assets', 'depreciation_and_depletion', 'net_increase_decrease_in_allowances_inventory_operating_activities', 'cash_flows_provided_from_used_in_financing_activities_correction', 'investment_tax_credit_adjustments_net', 'ending_balance', 'cash_flows_provided_from_used_in_financing_activities', 'net_increase_decrease_in_receivables_operating_activities', 'net_cash_flow_from_operating_activities_correction', 'cash_provided_by_outside_sources_correction', 'other_construction_and_acquisition_of_plant_investment_activities', 'disposition_of_investments_in_and_advances_to_associated_and_subsidiary_companies', 'undistributed_earnings_from_subsidiary_companies_operating_activities', 'cash_outflows_for_plant_correction', 'proceeds_from_issuance_of_preferred_stock_financing_activities', 'payments_for_retirement_of_preferred_stock_financing_activities', 'acquisition_of_other_noncurrent_assets', 'net_increase_decrease_in_other_regulatory_liabilities_operating_activities', 'investments_in_and_advances_to_associated_and_subsidiary_companies', 'gross_additions_to_utility_plant_less_nuclear_fuel_investing_activities', 'proceeds_from_issuance_of_long_term_debt_financing_activities', 'loans_made_or_purchased', 'other_adjustments_to_cash_flows_from_operating_activities', 'other_adjustments_to_cash_flows_from_investment_activities', 'cash_outflows_for_plant', 'gross_additions_to_nonutility_plant_investing_activities', 'payments_for_retirement_of_common_stock_financing_activities', 'proceeds_from_sales_of_investment_securities', 'net_income_loss', 'proceeds_from_issuance_of_common_stock_financing_activities', 'purchase_of_investment_securities', 'net_cash_flow_from_operating_activities', 'net_increase_decrease_in_inventory_operating_activities', 'payments_for_retirement_abstract'), nullable=False, comment='Label describing the type of amount being reported. This could be a balance or a change in value.'),
    ...
    sa.Column('utility_id_ferc1', sa.Integer(), nullable=False, comment='PUDL-assigned utility ID, identifying a FERC1 utility. This is an auto-incremented ID and is not expected to be stable from year to year.'),
    sa.Column('report_year', sa.Integer(), nullable=False, comment='Four-digit year in which the data was reported.'),
    sa.Column('depreciation_type', sa.Enum('accumulated_depreciation_correction', 'accumulated_depreciation', 'accumulated_depreciation_subdimension_correction'), nullable=False, comment='Type of depreciation provision within FERC Account 108, including cost ofremoval, depreciation expenses, salvage, cost of retired plant, etc.'),
    ...
    sa.Column('utility_id_ferc1', sa.Integer(), nullable=False, comment='PUDL-assigned utility ID, identifying a FERC1 utility. This is an auto-incremented ID and is not expected to be stable from year to year.'),
    sa.Column('report_year', sa.Integer(), nullable=False, comment='Four-digit year in which the data was reported.'),
    sa.Column('depreciation_type', sa.Enum('other_clearing_accounts_subdimension_correction', 'book_cost_of_asset_retirement_costs', 'book_cost_of_asset_retirement_costs_correction', 'other_adjustments_to_accumulated_depreciation_subdimension_correction', 'ending_balance_correction', 'ending_balance_subdimension_correction', 'book_cost_of_retired_plant', 'expenses_of_electric_plant_leased_to_others_subdimension_correction', 'ending_balance', 'cost_of_removal_of_plant', 'starting_balance_subdimension_correction', 'other_accounts_correction', 'expenses_of_electric_plant_leased_to_others', 'depreciation_provision_correction', 'book_cost_of_retired_plant_subdimension_correction', 'book_cost_of_asset_retirement_costs_subdimension_correction', 'depreciation_provision', 'cost_of_removal_of_plant_correction', 'depreciation_provision_subdimension_correction', 'expenses_of_electric_plant_leased_to_others_correction', 'salvage_value_of_retired_plant', 'depreciation_expense_excluding_adjustments', 'other_clearing_accounts_correction', 'salvage_value_of_retired_plant_correction', 'transportation_expenses_clearing_subdimension_correction', 'net_charges_for_retired_plant_subdimension_correction', 'depreciation_expense_excluding_adjustments_correction', 'depreciation_expense_for_asset_retirement_costs_correction', 'starting_balance', 'book_cost_of_retired_plant_correction', 'other_adjustments_to_accumulated_depreciation_correction', 'other_accounts', 'starting_balance_correction', 'other_accounts_subdimension_correction', 'salvage_value_of_retired_plant_subdimension_correction', 'depreciation_expense_for_asset_retirement_costs', 'other_adjustments_to_accumulated_depreciation', 'transportation_expenses_clearing', 'net_charges_for_retired_plant', 'depreciation_expense_excluding_adjustments_subdimension_correction', 'transportation_expenses_clearing_correction', 'depreciation_expense_for_asset_retirement_costs_subdimension_correction', 'other_clearing_accounts', 'net_charges_for_retired_plant_correction', 'cost_of_removal_of_plant_subdimension_correction'), nullable=False, comment='Type of depreciation provision within FERC Account 108, including cost ofremoval, depreciation expenses, salvage, cost of retired plant, etc.'),
    sa.Column('plant_status', sa.Text(), nullable=False, comment='Utility plant financial status (in service, future, leased, total).'),
    sa.Column('utility_type', sa.Text(), nullable=False, comment='Listing of utility plant types. Examples include Electric Utility, Gas Utility, and Other Utility.'),
    ...
    sa.Column('utility_id_ferc1', sa.Integer(), nullable=False, comment='PUDL-assigned utility ID, identifying a FERC1 utility. This is an auto-incremented ID and is not expected to be stable from year to year.'),
    sa.Column('plant_function', sa.Text(), nullable=False, comment='Functional role played by utility plant (steam production, nuclear production, distribution, transmission, etc.).'),
    sa.Column('ferc_account_label', sa.Enum('depreciation_amortization_total_correction', 'depreciation_amortization_total_subdimension_correction', 'depreciation_expense', 'depreciation_expense_correction', 'amortization_other_electric_plant_subdimension_correction', 'amortization_limited_term_electric_plant_subdimension_correction', 'depreciation_amortization_total', 'depreciation_expense_subdimension_correction', 'depreciation_expense_asset_retirement_correction', 'amortization_limited_term_electric_plant_correction', 'amortization_other_electric_plant', 'depreciation_expense_asset_retirement_subdimension_correction', 'amortization_other_electric_plant_correction', 'amortization_limited_term_electric_plant', 'depreciation_expense_asset_retirement'), nullable=False, comment='Long FERC account identifier derived from values reported in the XBRL taxonomies. May also refer to aggregations of individual FERC accounts.'),
    sa.Column('ferc_account', sa.Text(), nullable=True, comment="Actual FERC Account number (e.g. '359.1') if available, or a PUDL assigned ID when FERC accounts have been split or combined in reporting."),
    sa.Column('dollar_value', sa.Float(), nullable=True, comment='Dollar value of reported income, expense, asset, or liability.'),
    sa.Column('utility_type', sa.Text(), nullable=True, comment='Listing of utility plant types. Examples include Electric Utility, Gas Utility, and Other Utility.'),
    sa.Column('row_type_xbrl', sa.Enum('calculated_value', 'reported_value', 'correction', 'subdimension_correction'), nullable=True, comment='Indicates whether the value reported in the row is calculated, or uniquely reported within the table.'),
    ...
    sa.Column('report_year', sa.Integer(), nullable=False, comment='Four-digit year in which the data was reported.'),
    sa.Column('dollar_value', sa.Float(), nullable=True, comment='Dollar value of reported income, expense, asset, or liability.'),
    sa.Column('expense_type', sa.Enum('miscellaneous_general_expenses', 'distribution_operation_expenses_electric_correction', 'load_dispatch_transmission_service_and_scheduling', 'rents_other_power_generation', 'load_dispatching_transmission_expense', 'customer_account_expenses', 'maintenance_supervision_and_engineering', 'supervision_customer_service_and_information_expenses', 'power_production_expenses_correction', 'regulatory_commission_expenses', 'operation_supervision_and_engineering_steam_power_generation', 'maintenance_of_overhead_lines_transmission', 'maintenance_of_miscellaneous_other_power_generation_plant', 'maintenance_of_structures', 'generation_interconnection_studies', 'rents_nuclear_power_generation', 'distribution_operation_expenses_electric', 'reliability_planning_and_standards_development_services', 'allowances', 'nuclear_power_generation_operations_expense_correction', 'miscellaneous_transmission_expenses', 'maintenance_of_station_equipment_transmission', 'hydraulic_power_generation_operations_expense_correction', 'operation_supervision', 'power_purchased_for_storage_operations', 'maintenance_of_communication_equipment_electric_transmission', 'administrative_and_general_salaries', 'electric_expenses_nuclear_power_generation', 'other_expenses_other_power_supply_expenses', 'operation_of_energy_storage_equipment', 'administrative_and_general_expenses_correction', 'miscellaneous_customer_accounts_expenses', 'other_power_generation_maintenance_expense_correction', 'advertising_expenses', 'rents_steam_power_generation', 'street_lighting_and_signal_system_expenses', 'operations_and_maintenance_expenses_electric', 'reliability_planning_and_standards_development', 'steam_power_generation_operations_expense_correction', 'maintenance_of_energy_storage_equipment', 'station_expenses_distribution', 'generation_expenses', 'maintenance_of_electric_plant_nuclear_power_generation', 'maintenance_of_underground_lines', 'sales_expenses', 'market_facilitation_monitoring_and_compliance_services', 'steam_power_generation_maintenance_expense', 'maintenance_of_meters', 'maintenance_of_structures_and_improvements_regional_market_expenses', 'distribution_expenses', 'capacity_market_administration', 'fuel', 'maintenance_of_electric_plant_steam_power_generation', 'market_monitoring_and_compliance', 'regional_market_operation_expense_correction', 'regional_market_operation_expense', 'steam_power_generation_operations_expense', 'scheduling_system_control_and_dispatch_services', 'miscellaneous_customer_service_and_informational_expenses', 'maintenance_of_computer_software_transmission', 'maintenance_supervision_and_engineering_nuclear_power_generation', 'miscellaneous_sales_expenses', 'system_control_and_load_dispatching_electric', 'maintenance_of_structures_distribution_expense', 'customer_installations_expenses', 'regional_market_maintenance_expense_correction', 'power_production_expenses_hydraulic_power_correction', 'customer_service_and_information_expenses_correction', 'other_power_generation_operations_expense', 'hydraulic_power_generation_operations_expense', 'other_power_generation_maintenance_expense', 'duplicate_charges_credit', 'rents_distribution_expense', 'operation_of_energy_storage_equipment_distribution', 'steam_expenses_nuclear_power_generation', 'operation_supervision_and_engineering_distribution_expense', 'office_supplies_and_expenses', 'maintenance_of_computer_hardware', 'customer_records_and_collection_expenses', 'rents_administrative_and_general_expense', 'sales_expenses_correction', 'maintenance_supervision_and_engineering_other_power_generation', 'fuel_steam_power_generation', 'employee_pensions_and_benefits', 'steam_from_other_sources', 'miscellaneous_hydraulic_power_generation_expenses', 'power_production_expenses_steam_power', 'nuclear_power_generation_maintenance_expense_correction', 'regional_market_expenses', 'steam_expenses_steam_power_generation', 'maintenance_of_line_transformers', 'maintenance_of_energy_storage_equipment_other_power_generation', 'customer_account_expenses_correction', 'day_ahead_and_real_time_market_administration', 'transmission_of_electricity_by_others', 'uncollectible_accounts', 'franchise_requirements', 'maintenance_of_structures_hydraulic_power_generation', 'miscellaneous_nuclear_power_expenses', 'maintenance_of_computer_software', 'maintenance_of_miscellaneous_market_operation_plant', 'nuclear_power_generation_maintenance_expense', 'steam_transferred_credit', 'maintenance_of_communication_equipment_regional_market_expenses', 'underground_line_expenses', 'other_power_generation_operations_expense_correction', 'administrative_and_general_operation_expense_correction', 'meter_reading_expenses', 'rents_hydraulic_power_generation', 'other_power_supply_expense_correction', 'transmission_service_studies', 'maintenance_of_generating_and_electric_plant', 'maintenance_of_miscellaneous_distribution_plant', 'administrative_and_general_operation_expense', 'distribution_expenses_correction', 'transmission_operation_expense_correction', 'rents_regional_market_expenses', 'power_production_expenses', 'operation_supervision_and_engineering_electric_transmission_expenses', 'transmission_expenses', 'maintenance_of_miscellaneous_transmission_plant', 'miscellaneous_steam_power_expenses', 'property_insurance', 'power_production_expenses_nuclear_power_correction', 'transmission_expenses_correction', 'transmission_rights_market_administration', 'customer_service_and_information_expenses', 'maintenance_of_reactor_plant_equipment_nuclear_power_generation', 'general_advertising_expenses', 'maintenance_of_boiler_plant_steam_power_generation', 'maintenance_of_energy_storage_equipment_transmission', 'hydraulic_expenses', 'load_dispatch_monitor_and_operate_transmission_system', 'maintenance_of_miscellaneous_hydraulic_plant', 'overhead_line_expenses', 'nuclear_power_generation_operations_expense', 'load_dispatch_reliability', 'maintenance_of_miscellaneous_regional_transmission_plant', 'steam_transferred_credit_nuclear_power_generation', 'hydraulic_power_generation_maintenance_expense', 'customer_assistance_expenses', 'nuclear_fuel_expense', 'demonstrating_and_selling_expenses', 'miscellaneous_distribution_expenses', 'maintenance_of_station_equipment', 'maintenance_supervision_and_engineering_hydraulic_power_generation', 'meter_expenses', 'power_production_expenses_other_power_correction', 'administrative_expenses_transferred_credit', 'distribution_maintenance_expense_electric', 'operation_supervision_and_engineering_other_power_generation', 'transmission_maintenance_expense_electric_correction', 'rents_transmission_electric_expense', 'supervision_sales_expense', 'power_production_expenses_other_power', 'station_expenses_transmission_expense', 'maintenance_supervision_and_engineering_electric_transmission_expenses', 'operation_of_energy_storage_equipment_transmission_expense', 'purchased_power', 'maintenance_of_structures_nuclear_power_generation', 'electric_expenses_hydraulic_power_generation', 'maintenance_of_street_lighting_and_signal_systems', 'transmission_maintenance_expense_electric', 'regional_market_expenses_correction', 'steam_power_generation_maintenance_expense_correction', 'administrative_and_general_expenses', 'water_for_power', 'operations_and_maintenance_expenses_electric_correction', 'injuries_and_damages', 'maintenance_of_structures_transmission_expense', 'miscellaneous_other_power_generation_expenses', 'power_production_expenses_steam_power_correction', 'maintenance_of_miscellaneous_nuclear_plant', 'underground_line_expenses_transmission_expense', 'maintenance_of_underground_lines_transmission', 'steam_from_other_sources_nuclear_power_generation', 'maintenance_of_structures_steam_power_generation', 'informational_and_instructional_advertising_expenses', 'distribution_maintenance_expense_electric_correction', 'maintenance_of_reservoirs_dams_and_waterways', 'ancillary_services_market_administration', 'other_power_supply_expense', 'outside_services_employed', 'power_production_expenses_nuclear_power', 'transmission_operation_expense', 'electric_expenses_steam_power_generation', 'overhead_line_expense', 'power_production_expenses_hydraulic_power', 'operation_supervision_and_engineering_nuclear_power_generation', 'maintenance_of_computer_hardware_transmission', 'maintenance_of_general_plant', 'supervision_customer_account_expenses', 'maintenance_of_overhead_lines', 'maintenance_of_electric_plant_hydraulic_power_generation', 'hydraulic_power_generation_maintenance_expense_correction', 'maintenance_of_miscellaneous_steam_plant', 'coolants_and_water', 'load_dispatching', 'regional_market_maintenance_expense', 'maintenance_supervision_and_engineering_steam_power_generation', 'operation_supervision_and_engineering_hydraulic_power_generation'), nullable=False, comment='The type of expense.'),
    sa.Column('record_id', sa.Text(), nullable=True, comment='Identifier indicating original FERC Form 1 source record. format: {table_name}_{report_year}_{report_prd}_{respondent_id}_{spplmnt_num}_{row_number}. Unique within FERC Form 1 DB tables which are not row-mapped.'),
    sa.Column('utility_type', sa.Text(), nullable=True, comment='Listing of utility plant types. Examples include Electric Utility, Gas Utility, and Other Utility.'),
    sa.Column('ferc_account', sa.Text(), nullable=True, comment="Actual FERC Account number (e.g. '359.1') if available, or a PUDL assigned ID when FERC accounts have been split or combined in reporting."),
    sa.Column('row_type_xbrl', sa.Enum('calculated_value', 'reported_value', 'correction', 'subdimension_correction'), nullable=True, comment='Indicates whether the value reported in the row is calculated, or uniquely reported within the table.'),
    ...
    sa.Column('report_year', sa.Integer(), nullable=False, comment='Four-digit year in which the data was reported.'),
    sa.Column('record_id', sa.Text(), nullable=True, comment='Identifier indicating original FERC Form 1 source record. format: {table_name}_{report_year}_{report_prd}_{respondent_id}_{spplmnt_num}_{row_number}. Unique within FERC Form 1 DB tables which are not row-mapped.'),
    sa.Column('revenue_type', sa.Enum('sales_to_railroads_and_railways', 'sales_of_electricity', 'interdepartmental_sales_correction', 'residential_sales_correction', 'revenues_net_of_provision_for_refunds', 'electric_operating_revenues_correction', 'other_operating_revenues', 'small_or_commercial', 'revenues_from_transmission_of_electricity_of_others', 'large_or_industrial', 'sales_for_resale_correction', 'sales_of_water_and_water_power', 'public_street_and_highway_lighting', 'other_operating_revenues_correction', 'rent_from_electric_property', 'electric_operating_revenues', 'sales_to_railroads_and_railways_correction', 'miscellaneous_revenue', 'other_electric_revenue', 'revenues_net_of_provision_for_refunds_correction', 'interdepartmental_sales', 'residential_sales', 'other_miscellaneous_operating_revenues', 'small_or_commercial_correction', 'other_sales_to_public_authorities', 'miscellaneous_service_revenues', 'other_sales_to_public_authorities_correction', 'forfeited_discounts', 'large_or_industrial_correction', 'provision_for_rate_refunds', 'public_street_and_highway_lighting_correction', 'sales_to_ultimate_consumers', 'interdepartmental_rents', 'regional_transmission_service_revenues', 'sales_for_resale', 'sales_of_electricity_correction', 'sales_to_ultimate_consumers_correction'), nullable=False, comment='Label describing types of revenues.'),
    sa.Column('dollar_value', sa.Float(), nullable=True, comment='Dollar value of reported income, expense, asset, or liability.'),
    sa.Column('sales_mwh', sa.Float(), nullable=True, comment='Quantity of electricity sold in MWh.'),
    ...
    sa.Column('utility_id_ferc1', sa.Integer(), nullable=False, comment='PUDL-assigned utility ID, identifying a FERC1 utility. This is an auto-incremented ID and is not expected to be stable from year to year.'),
    sa.Column('report_year', sa.Integer(), nullable=False, comment='Four-digit year in which the data was reported.'),
    sa.Column('ferc_account_label', sa.Enum('meters_distribution_plant', 'land_and_land_rights_steam_production', 'station_equipment_distribution_plant', 'accessory_electric_equipment_other_production', 'laboratory_equipment_general_plant', 'land_and_land_rights_distribution_plant', 'structures_and_improvements_steam_production', 'turbogenerator_units_nuclear_production', 'organization', 'transmission_plant_correction', 'underground_conductors_and_devices_transmission_plant', 'transmission_plant', 'accessory_electric_equipment_steam_production', 'hydraulic_production_plant_correction', 'leased_property_on_customer_premises_distribution_plant', 'tools_shop_and_garage_equipment_general_plant', 'communication_equipment_regional_transmission_and_market_operation_plant', 'boiler_plant_equipment_steam_production', 'nuclear_production_plant_correction', 'street_lighting_and_signal_systems_distribution_plant', 'computer_software_regional_transmission_and_market_operation_plant', 'miscellaneous_power_plant_equipment_steam_production', 'distribution_plant_correction', 'prime_movers_other_production', 'poles_and_fixtures_transmission_plant', 'asset_retirement_costs_for_other_production_plant_other_production', 'production_plant_correction', 'roads_railroads_and_bridges_hydraulic_production', 'overhead_conductors_and_devices_distribution_plant', 'steam_production_plant_correction', 'other_production_plant_correction', 'structures_and_improvements_hydraulic_production', 'asset_retirement_costs_for_regional_transmission_and_market_operation_plant_regional_transmission_and_market_operation_plant', 'underground_conductors_and_devices_distribution_plant', 'office_furniture_and_equipment_general_plant', 'electric_plant_in_service_and_completed_construction_not_classified_electric_correction', 'line_transformers_distribution_plant', 'water_wheels_turbines_and_generators_hydraulic_production', 'miscellaneous_power_plant_equipment_nuclear_production', 'electric_plant_purchased', 'franchises_and_consents', 'land_and_land_rights_hydraulic_production', 'engines_and_engine_driven_generators_steam_production', 'miscellaneous_regional_transmission_and_market_operation_plant_regional_transmission_and_market_operation_plant', 'roads_and_trails_transmission_plant', 'other_production_plant', 'land_and_land_rights_other_production', 'accessory_electric_equipment_nuclear_production', 'installations_on_customer_premises_distribution_plant', 'underground_conduit_distribution_plant', 'miscellaneous_power_plant_equipment_hydraulic_production', 'asset_retirement_costs_for_transmission_plant_transmission_plant', 'structures_and_improvements_nuclear_production', 'land_and_land_rights_regional_transmission_and_market_operation_plant', 'general_plant_correction', 'production_plant', 'land_and_land_rights_nuclear_production', 'asset_retirement_costs_for_general_plant_general_plant', 'structures_and_improvements_general_plant', 'structures_and_improvements_regional_transmission_and_market_operation_plant', 'other_tangible_property_general_plant', 'intangible_plant_correction', 'transmission_and_market_operation_plant_regional_transmission_and_market_operation_plant', 'electric_plant_in_service_and_completed_construction_not_classified_electric', 'hydraulic_production_plant', 'structures_and_improvements_distribution_plant', 'asset_retirement_costs_for_steam_production_plant_steam_production', 'electric_plant_in_service_correction', 'generators_other_production', 'land_and_land_rights_transmission_plant', 'energy_storage_equipment_other_production', 'nuclear_production_plant', 'energy_storage_equipment_distribution_plant', 'communication_equipment_general_plant', 'reservoirs_dams_and_waterways_hydraulic_production', 'stores_equipment_general_plant', 'land_and_land_rights_general_plant', 'structures_and_improvements_transmission_plant', 'poles_towers_and_fixtures_distribution_plant', 'asset_retirement_costs_for_nuclear_production_plant_nuclear_production', 'asset_retirement_costs_for_hydraulic_production_plant_hydraulic_production', 'miscellaneous_power_plant_equipment_other_production', 'station_equipment_transmission_plant', 'overhead_conductors_and_devices_transmission_plant', 'transmission_and_market_operation_plant_regional_transmission_and_market_operation_plant_correction', 'experimental_electric_plant_unclassified', 'general_plant_excluding_other_tangible_property_and_asset_retirement_costs_for_general_plant', 'energy_storage_equipment_transmission_plant', 'fuel_holders_products_and_accessories_other_production', 'reactor_plant_equipment_nuclear_production', 'steam_production_plant', 'miscellaneous_equipment_general_plant', 'distribution_plant', 'computer_hardware_regional_transmission_and_market_operation_plant', 'towers_and_fixtures_transmission_plant', 'underground_conduit_transmission_plant', 'asset_retirement_costs_for_distribution_plant_distribution_plant', 'intangible_plant', 'transportation_equipment_general_plant', 'general_plant_excluding_other_tangible_property_and_asset_retirement_costs_for_general_plant_correction', 'miscellaneous_intangible_plant', 'structures_and_improvements_other_production', 'electric_plant_sold', 'electric_plant_in_service', 'services_distribution_plant', 'power_operated_equipment_general_plant', 'general_plant', 'accessory_electric_equipment_hydraulic_production', 'turbogenerator_units_steam_production'), nullable=False, comment='Long FERC account identifier derived from values reported in the XBRL taxonomies. May also refer to aggregations of individual FERC accounts.'),
    sa.Column('ferc_account', sa.Text(), nullable=True, comment="Actual FERC Account number (e.g. '359.1') if available, or a PUDL assigned ID when FERC accounts have been split or combined in reporting."),
    sa.Column('row_type_xbrl', sa.Enum('calculated_value', 'reported_value', 'correction', 'subdimension_correction'), nullable=True, comment='Indicates whether the value reported in the row is calculated, or uniquely reported within the table.'),
    sa.Column('starting_balance', sa.Float(), nullable=True, comment='Account balance at beginning of year.'),
    ...
    sa.Column('report_year', sa.Integer(), nullable=False, comment='Four-digit year in which the data was reported.'),
    sa.Column('record_id', sa.Text(), nullable=True, comment='Identifier indicating original FERC Form 1 source record. format: {table_name}_{report_year}_{report_prd}_{respondent_id}_{spplmnt_num}_{row_number}. Unique within FERC Form 1 DB tables which are not row-mapped.'),
    sa.Column('earnings_type', sa.Enum('appropriated_retained_earnings_including_reserve_amortization_correction', 'unappropriated_retained_earnings', 'adjustments_to_retained_earnings_credit', 'dividends_received', 'dividends_declared_common_stock', 'adjustments_to_retained_earnings_debit', 'appropriated_retained_earnings', 'changes_unappropriated_undistributed_subsidiary_earnings_credits', 'appropriations_of_retained_earnings', 'appropriated_retained_earnings_including_reserve_amortization', 'transfers_from_unappropriated_undistributed_subsidiary_earnings', 'unappropriated_undistributed_subsidiary_earnings', 'unappropriated_retained_earnings_previous_year', 'unappropriated_undistributed_subsidiary_earnings_previous_year', 'retained_earnings_correction', 'unappropriated_undistributed_subsidiary_earnings_correction', 'equity_in_earnings_of_subsidiary_companies', 'appropriated_retained_earnings_amortization_reserve_federal', 'unappropriated_retained_earnings_correction', 'balance_transferred_from_income', 'dividends_declared_preferred_stock', 'retained_earnings'), nullable=False, comment='Label describing types of earnings.'),
    sa.Column('starting_balance', sa.Float(), nullable=True, comment='Account balance at beginning of year.'),
    sa.Column('ending_balance', sa.Float(), nullable=True, comment='Account balance at end of year.'),
    sa.Column('balance', sa.Text(), nullable=True, comment='Indication of whether a column is a credit or debit, as reported in the XBRL taxonomy.'),
    sa.Column('ferc_account', sa.Text(), nullable=True, comment="Actual FERC Account number (e.g. '359.1') if available, or a PUDL assigned ID when FERC accounts have been split or combined in reporting."),
    ...
    sa.Column('report_year', sa.Integer(), nullable=True, comment='Four-digit year in which the data was reported.'),
    sa.Column('plant_name_ferc1', sa.Text(), nullable=True, comment='Name of the plant, as reported to FERC. This is a freeform string, not guaranteed to be consistent across references to the same plant.'),
    sa.Column('plant_type', sa.Enum('steam', 'na_category', 'internal_combustion', 'solar_thermal', 'combustion_turbine', 'nuclear', 'geothermal', 'photovoltaic', 'combined_cycle', 'wind'), nullable=True, comment='Type of plant.'),
    sa.Column('construction_type', sa.Enum('conventional', 'outdoor', 'semioutdoor'), nullable=True, comment="Type of plant construction ('outdoor', 'semioutdoor', or 'conventional'). Categorized by PUDL based on our best guess of intended value in FERC1 freeform strings."),
    sa.Column('construction_year', sa.Integer(), nullable=True, comment="Year the plant's oldest still operational unit was built."),
    sa.Column('installation_year', sa.Integer(), nullable=True, comment="Year the plant's most recently built unit was installed."),
    sa.Column('capacity_mw', sa.Float(), nullable=True, comment='Total installed (nameplate) capacity, in megawatts.'),
    ...
    sa.Column('owner_state', sa.Enum('KS', 'MT', 'YT', 'QC', 'GA', 'IA', 'AZ', 'IL', 'OK', 'MD', 'SD', 'CA', 'AL', 'GU', 'AS', 'NB', 'NH', 'NT', 'CO', 'FL', 'AK', 'KY', 'IN', 'DE', 'OR', 'AB', 'ME', 'RI', 'MN', 'NV', 'BC', 'MB', 'NS', 'MO', 'PA', 'AR', 'ID', 'WV', 'NL', 'DC', 'HI', 'MI', 'SK', 'ON', 'NM', 'NC', 'ND', 'PR', 'TN', 'UT', 'VA', 'WI', 'VI', 'NJ', 'NY', 'LA', 'CT', 'PE', 'NU', 'OH', 'TX', 'MP', 'VT', 'WA', 'NE', 'WY', 'MS', 'MA', 'SC'), nullable=True, comment='Two letter ISO-3166 political subdivision code.'),
    ...
    sa.Column('plant_id_eia', sa.Integer(), nullable=True, comment='The unique six-digit facility identification number, also called an ORISPL, assigned by the Energy Information Administration.'),
    sa.Column('report_date', sa.Date(), nullable=True, comment='Date reported.'),
    sa.Column('plant_part', sa.Enum('plant_prime_fuel', 'plant_unit', 'plant_match_ferc1', 'plant', 'plant_technology', 'plant_gen', 'plant_prime_mover', 'plant_operating_year', 'plant_ferc_acct'), nullable=True, comment='The part of the plant a record corresponds to.'),
    sa.Column('generator_id', sa.Text(), nullable=True, comment='Generator ID is usually numeric, but sometimes includes letters. Make sure you treat it as a string!'),
    sa.Column('unit_id_pudl', sa.Integer(), nullable=True, comment='Dynamically assigned PUDL unit id. WARNING: This ID is not guaranteed to be static long term as the input data and algorithm may evolve over time.'),
    sa.Column('prime_mover_code', sa.Text(), nullable=True, comment='Code for the type of prime mover (e.g. CT, CG)'),
    sa.Column('energy_source_code_1', sa.Text(), nullable=True, comment='The code representing the most predominant type of energy that fuels the generator.'),
    ...
    sa.Column('appro_part_label', sa.Enum('plant_prime_fuel', 'plant_unit', 'plant_match_ferc1', 'plant', 'plant_technology', 'plant_gen', 'plant_prime_mover', 'plant_operating_year', 'plant_ferc_acct'), nullable=True, comment='Plant part of the associated true granularity record.'),
