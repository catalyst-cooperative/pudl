...
def _plants_steam_assign_plant_ids(ferc1_steam_df, ferc1_fuel_df):
    """Assigns IDs to the large steam plants.
    """
    ###########################################################################
...
def plants_steam_validate_ids(ferc1_steam_df):
    """Tests that the plant_id_ferc1 times series includes one record per year.
    """
    ##########################################################################
...
class FERCPlantClassifier(BaseEstimator, ClassifierMixin):
    ...
    def __init__(self, min_sim=0.75, plants_df=None):
        ...
        Args:
            min_sim : Number between 0.0 and 1.0, indicating the minimum value
                of cosine similarity that we are willing to accept as indicating
                two records are part of the same plant record time series. All
                entries in the pairwise similarity matrix below this value will
                be zeroed out.
    ...
    def fit(self, X, y=None):
        """Uses weighted FERC plant features to group records into time series.
