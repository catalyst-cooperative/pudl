...
class HourlyEmissions(pudl.models.entities.PUDLBase):
    ...
    plant_id_eia = Column(Integer, nullable=False, comment="EIA Office of the Regulatory Information System Plant Identification number. One to five digit numeric.")
    unitid = Column(String, nullable=False, comment="Facility-specific unit id (e.g. Unit 4)")
    # SQLA recommends TIMESTAMP over DateTime when dealing with timezones
    operating_datetime_utc = Column(TIMESTAMP(timezone=True), nullable=False, comment="Date and time measurement began.")
    operating_time_hours = Column(REAL, comment="Length of time interval measured.")
    gross_load_mw = Column(REAL, nullable=False, comment="Power delivered during time interval measured.")
    steam_load_1000_lbs = Column(REAL, comment="Total steam pressure produced by a unit or source in any calendar year (or other specified time period) produced by combusting a given heat input of fuel.")
    so2_mass_lbs = Column(REAL, comment="Sulfur dioxide emissions in pounds.")
    so2_mass_measurement_code = Column(ENUM_FLAG_MEASUREMENT, comment="Identifies whether the reported value of emissions was measured, calculated, or measured and substitute.")
    # so2_rate_lbs_mmbtu = Column(REAL)
    # so2_rate_measure_flg = Column(ENUM_FLAG_CALCULATED)
    nox_rate_lbs_mmbtu = Column(REAL, comment="The average rate at which NOx was emitted during a given time period.")
    nox_rate_measurement_code = Column(ENUM_NOX, comment="Identifies whether the reported value of emissions was measured, calculated, or measured and substitute.")
    nox_mass_lbs = Column(REAL, comment="Nitrogen oxide emissions in pounds.")
    nox_mass_measurement_code = Column(ENUM_NOX, comment="Identifies whether the reported value of emissions was measured, calculated, or measured and substitute.")
    co2_mass_tons = Column(REAL, comment="Carbon dioxide emissions in short tons.")
    co2_mass_measurement_code = Column(ENUM_FLAG_MEASUREMENT, comment="Identifies whether the reported value of emissions was measured, calculated, or measured and substitute.")
    # co2_rate_tons_mmbtu = Column(REAL)
    # co2_rate_measure_flg = Column(ENUM_FLAG_CALCULATED)
    heat_content_mmbtu = Column(REAL, nullable=False, comment="The measure of utilization that is calculated by multiplying the quantity of fuel by the fuel's heat content.")
    facility_id = Column(SmallInteger, comment="The unique six-digit facility identification number, also called an ORISPL, assigned by the Energy Information Administration.")  # max value is 8421
    unit_id_epa = Column(Integer, comment="Unique EPA identifier for each unit at a facility.")
