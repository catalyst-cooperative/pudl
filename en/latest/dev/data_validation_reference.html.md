...
be reused across multiple tables. dbt includes a few [built-in data test definitions](https://docs.getdbt.com/docs/build/data-tests), and the [dbt_expectations](https://github.com/metaplane/dbt-expectations) package provides many more. We also
define our own [custom data tests](https://docs.getdbt.com/best-practices/writing-custom-generic-tests).
...
* From dbt’s point of view, the PUDL tables are all
  [sources](https://docs.getdbt.com/docs/build/sources) – external tables about
  ...
  most tables would be defined as [models](https://docs.getdbt.com/docs/build/models)
  which are somewhat analogous to [Dagster assets](https://docs.dagster.io/guides/build/assets/defining-assets).
* As a SQL-based tool, dbt generally expects to be querying a database. However, in our
  ...
  use [the dbt adapter object](https://docs.getdbt.com/reference/dbt-jinja-functions/adapter) in our test
...
* One place we use true dbt models instead of sources is when
  ...
  See [Creating intermediate tables for a test](#intermediate-tables).
...
To initiate a branch build, in the PUDL repo on GitHub go to [Actions](https://github.com/catalyst-cooperative/pudl/actions) and select [build-pudl](https://github.com/catalyst-cooperative/pudl/actions/workflows/build-pudl.yml).
...
track its progress and watch the logs in the [Google Cloud Console](https://console.cloud.google.com/monitoring/dashboards/builder/992bbe3f-17e6-49c4-a9e8-8f1925d4ec24).
...
dbt has its own much more [extensive documentation](https://docs.getdbt.com/).
...
For more options, see the [dbt selection syntax documentation](https://docs.getdbt.com/reference/node-selection/syntax).
...
run the SQL yourself, and explore duckdb’s [output formatting options](https://duckdb.org/docs/stable/clients/cli/output_formats.html) to display
the results legibly.
...
There are lots of [output format modes available](https://duckdb.org/docs/stable/clients/cli/output_formats.html); hopefully one of them will be legible for your failure row output!
...
Our dbt project includes [dbt-utils](https://github.com/dbt-labs/dbt-utils) and
[dbt-expectations](https://github.com/metaplane/dbt-expectations) as dependencies.
...
Our dbt schemas are defined within schema files at
`dbt/models/<data_source>/<table_name>/schema.yml`.
These are automatically generated by combining the resource
definitions in `pudl.metadata.resources.RESOURCE_METADATA`
with some human-generated data test definitions in
`dbt/schema_inputs/<data_source>/<table_name>/schema.human.yml`.

When adding a new table to dbt, we need to:

* add initial data tests
* generate the `schema.yml` and check it in

The new table must have already been added to the `RESOURCE_METADATA` and be
written out to Parquet in order for it to be available to dbt.

**Non-data-test related \`\`schema.human.yml\`\` keys (like table and column
descriptions) will not be applied to the final \`\`schema.yml\`\`, and will cause a
validation failure in schema generation.**
...
### Generating the `schema.yml`

```bash
dbt_helper update-tables --schema new_table_name
```

This will read the metadata from `RESOURCE_METADATA`, merge it with the
`schema.human.yml`, and write the combination out to `schema.yml` so dbt can
see it.

You can also give it a list of tables and they will all be created at once.
...
`dbt/tests/data_tests/generic_tests`) or the tests in [dbt_expectations](https://hub.getdbt.com/metaplane/dbt_expectations/latest/) or [dbt_utils](https://hub.getdbt.com/dbt-labs/dbt_utils/latest/).
...
the tools within Python. In those cases, you can use [Dagster’s asset checks](https://docs.dagster.io/guides/test/asset-checks), but in general we prefer
using dbt tests.
...
`pudl/dbt/tests/data_tests/generic_tests`. dbt has [official docs](https://docs.getdbt.com/best-practices/writing-custom-generic-tests) for
doing this, but the core steps are:

1. Check to see whether the test you need is already provided by [dbt-utils](https://hub.getdbt.com/dbt-labs/dbt_utils/latest/) or [dbt-expectations](https://github.com/metaplane/dbt-expectations).
...
* [Interactive Mode SQL Tutorial](https://mode.com/sql-tutorial)
* [Greg Wilson’s Querynomicon](https://third-bit.com/sql/)
* [Interactive DuckDB SQL Tutorial](https://motherduckdb.github.io/sql-tutorial/)
* [DuckDB SQL Introduction](https://duckdb.org/docs/stable/sql/introduction.html)
* [SQL for Data Scientists](https://www.oreilly.com/library/view/sql-for-data/9781119669364/) (book)

#### NOTE
Refer to [PUDL Specific Design Choices](#pudl-dbt-quirks) above for an explanation of some details of our dbt
...
This can be done by creating a new [dbt model](https://docs.getdbt.com/docs/build/models) that materializes an
