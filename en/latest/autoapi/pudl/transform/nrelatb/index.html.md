...
table has a null in this column, standing in for the `*` wildcard NREL uses in
the raw data (already converted to null upstream, in
[`_core_nrelatb__transform_start()`](#pudl.transform.nrelatb._core_nrelatb__transform_start)). This is a part of the tables primary key
and because we know a wildcard in a primary key effectively means “applies to
everything”, we want to broadcast these records across the rest of the data.
Unfortunately, there are still ~5% of records w/ a wildcard that don’t have
associated records in the rest of the data (the are left_only records in
[`_broadcast_core_metric_parameters()`](#pudl.transform.nrelatb._broadcast_core_metric_parameters)) so they end up with nulls in the
`cost_recovery_period_years` column.
