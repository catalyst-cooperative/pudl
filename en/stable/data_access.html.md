...
| [Kaggle](#access-kaggle)                           | Parquet, DuckDB, SQLite                                 | `nightly`                                                | Data Scientist, Data Analyst, Jupyter Notebook User                | Work with PUDL data products in Jupyter Notebooks via the web with minimal setup.<br/>Explore curated and contributed analyses and visualizations using PUDL data.<br/>notebooks.<br/>Create and share your own interactive notebooks using PUDL data.                                                                                  |
| [Cloud Storage](#access-cloud)                    | Parquet, DuckDB, SQLite                                 | `nightly`, `stable`                                      | Data Scientist, Analytics Engineer, Data Engineer, Cloud Developer | Performant remote queries of clearly versioned PUDL Parquet outputs from cloud<br/>computing platforms or GitHub Actions.<br/>Fast bulk download of outputs for local use.<br/>Parquet based data warehouse for large-scale data analysis in the cloud.<br/>Integrates well with Pandas, Polars, DuckDB, and other dataframe libraries. |
| [Zenodo Archives](#access-zenodo)                  | Parquet, DuckDB, SQLite                                 | `stable`                                                 | Researcher, Publisher, Archivist                                   | Access a specific, immutable version of the PUDL data by DOI for citation in<br/>academic publications or other applications where long-term reproducibility is<br/>needed. Web-based bulk download of data for local analysis.                                                                                                         |
| [Coding Agents (PUDL Agent Skill)](#access-agent-skill) | Parquet                                                 | `nightly`, `stable`                                      | Coding Agent User                                                  | Let an AI coding agent (Claude Code, OpenCode, Pi, etc.) discover PUDL tables,<br/>look up column meanings and data-quality caveats, and load the Parquet outputs<br/>into a notebook or script.                                                                                                                                        |
...
[Cloud Storage](#access-cloud), [Zenodo Archives](#access-zenodo), and the [PUDL agent skill](#access-agent-skill) for use with AI coding agents.
...
- [Apache Parquet](https://parquet.apache.org/docs/): a compressed, columnar storage
...
- [DuckDB](https://duckdb.org): a fast, self-contained analytical database that holds
...
[minimally processed raw FERC data](#access-raw-ferc), which will continue to be
...
- **SQLite only**: The [minimally processed FERC data](#access-raw-ferc) which we
  ...
  experimentally, DuckDB — see [Raw FERC XBRL data converted to DuckDB](#access-raw-ferc-duckdb)).
...
be downloaded from S3 (see [Cloud Storage](#access-cloud)) or our regular versioned releases (see
[Zenodo Archives](#access-zenodo)). All the converted FERC databases can also be accessed through
[PUDL Data Viewer](#access-viewer).
...
The [PUDL Data Viewer](https://data.catalyst.coop/) provides flexible search of table
...
locally with tools like [Tad](https://www.tadviewer.com/).
...
[DuckDB](https://duckdb.org/2021/06/25/querying-parquet.html) with the [httpfs
extension](https://duckdb.org/docs/guides/network_cloud_storage/s3_import.html) can
...
The same works from the [DuckDB Python API](https://duckdb.org/docs/stable/clients/python/overview):
...
run many queries — see [Direct Links for Bulk Download](#direct-download).

### Polars

[Polars](https://docs.pola.rs/) can scan the [Parquet outputs directly from S3](https://docs.pola.rs/user-guide/io/cloud-storage/), using lazy evaluation so that
...
The [arrow](https://arrow.apache.org/docs/r/) package lets you [open a Parquet file
directly from S3](https://www.pmassicotte.com/posts/2024-05-01-query-s3-duckplyr/)
with [dplyr](https://dplyr.tidyverse.org/) verbs. Column selection and row filters
...
* [Main PUDL Database](https://s3.us-west-2.amazonaws.com/pudl.catalyst.coop/nightly/pudl.duckdb)

#### Fully Processed SQLite Databases

#### WARNING
The fully processed `pudl.sqlite` database is deprecated and will no longer be
produced starting in 2027. See [Data Format](#access-format). Use the Parquet outputs or the
...
[catalyst-cooperative/agent-skills](https://github.com/catalyst-cooperative/agent-skills) repository on GitHub. The
...
installed. See the [skill’s README](https://github.com/catalyst-cooperative/agent-skills/tree/main/skills/pudl) for
