...
def test_make_mega_gen_tbl():
    ...
    mcoe = (
        ...
        .astype(
            {
                "generator_retirement_date": "datetime64[us]",
...
def test_scale_by_ownership():
    ...
    ).astype({"report_date": "datetime64[us]", "owner_utility_id_eia": pd.Int64Dtype()})

    dtypes = {"report_date": "datetime64[us]", "utility_id_eia": pd.Int64Dtype()}
...
def test_one_to_many():
    plant_part_list_input = pd.DataFrame(
    ...
    ).astype(
        {
            "report_date": "datetime64[us]",
            "generator_retirement_date": "datetime64[us]",
            "planned_generator_retirement_date": "datetime64[us]",
            "generator_id": "string",
        }
    )
...
def test_add_record_id_null_utility():
    """Records with a null utility_id_eia still get a non-null, unique record ID ending in ``null``.
