...
@pytest.mark.parametrize(
    ("run_git_side_effect", "expected_error"),
    [
        # tag and HEAD resolve to the same commit (also confirms .strip()).
        (["abc123def\n", "  abc123def  \n"], None),
        # tag and HEAD resolve to different commits.
        (["tagcommit0\n", "headcommit1\n"], "does not match"),
        # `git rev-parse <tag>` fails outright.
        (subprocess.CalledProcessError(128, ["git", "rev-parse"]), "Could not resolve"),
    ],
    ids=["match", "mismatch", "unresolvable"],
)
def test_verify_git_tag_checked_out(mocker, run_git_side_effect, expected_error):
    """Pin down the comparison logic downstream of ``run_git``.

    Real-git behavior is covered in tests/integration/deploy/; here we just fake
    ``run_git``'s output for each of the three outcomes.
    """
    mocker.patch("pudl.deploy.zenodo_metadata.run_git", side_effect=run_git_side_effect)
