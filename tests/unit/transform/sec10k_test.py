...
def test_match_ex21_subsidiaries_to_filer_company_tiebreak_is_order_independent():
    """A tie between equally-good filer candidates must resolve deterministically.

    Reproduces a bug where two candidate filer companies tied on every ranking
    criterion (name, location overlap, and report date proximity) resolved to
    whichever one happened to come first in the input, via
    ``groupby(...).first()`` with no further tiebreak. That's consistent
    within a single environment, but not guaranteed to match across
    pandas/numpy versions or platforms.
    """
    ownership_df_test = pd.read_csv(
        StringIO(
            """
subsidiary_company_id_sec10k,subsidiary_company_name,subsidiary_company_location,report_date
sub_1,widget co,texas,2020-01-01
sub_2,other filler co,,2020-01-01
"""
        ),
        parse_dates=["report_date"],
    )
    # Two filer candidates for "widget co" that are exactly tied: same
    # incorporation state and same report date, so identical loc_overlap and
    # report_date_diff_days. Only central_index_key differs between them.
    info_df_ascending = pd.read_csv(
        StringIO(
            """
central_index_key,company_name,incorporation_state,report_date
0000000010,widget co,TX,2020-06-01
0000000020,widget co,TX,2020-06-01
0000000099,other filler co,,2020-06-01
"""
        ),
        parse_dates=["report_date"],
    )
    info_df_descending = info_df_ascending.iloc[[1, 0, 2]].reset_index(drop=True)

    winners = {
        order_name: (
            _match_ex21_subsidiaries_to_filer_company(
                filer_info_df=info_df.copy(), ownership_df=ownership_df_test.copy()
            )
            .set_index("subsidiary_company_id_sec10k")
            .loc["sub_1", "subsidiary_company_central_index_key"]
        )
        for order_name, info_df in [
            ("ascending", info_df_ascending),
            ("descending", info_df_descending),
        ]
    }

    assert winners["ascending"] == winners["descending"]
